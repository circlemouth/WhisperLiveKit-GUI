...
- 左カラムの Server Settings では Start/Stop API ボタンを最下部に固定し、その上にウィンドウ高さに応じて伸縮するスペースを設ける
- 右カラムを Endpoints / Recorder / Logs の三段構成とし、ログ欄は Recorder の下部に配置。ステータス表示と進捗バーを廃止し、ログ欄は最低4行を維持しつつトランスクリプト欄と柔軟に高さを分配。画面全体を縦スクロール可能にし、ログ欄はウィンドウ縦方向に追随
  - 左右カラムの最小幅（左800px / 右360px）とウィンドウ最小サイズ（1200×800）を設定し、極端な縮小を防止。起動直後に左カラムが表示されない不具合を修正
  - ウィンドウ拡大後に左右カラムが伸びても、縮小時に高さがウィンドウに追随して UI 全体が常に表示されるよう ScrollableFrame を改修
