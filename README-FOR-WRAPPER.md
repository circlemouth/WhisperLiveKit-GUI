...
MSIX 配布時は、以下の証明書・信頼を区別して取り扱います。

- パッケージ署名（Publisher 証明書）: MSIX のインストールに必要。発行元の公開鍵（Publisher.cer）を「ローカル コンピューター > 信頼された発行元 (Trusted People)」へインポート。
- アプリ内 HTTPS（サーバー証明書）: バックエンド/API を HTTPS で提供する場合に使用。自己署名を使う場合はクライアントへ CER を信頼登録。
- 外向き TLS（CA ルート信頼）: GitHub/Hugging Face 等への HTTPS 接続検証に使用。企業プロキシ/私設 CA 環境では、その CA の CER を OS に信頼登録しないと失敗します。

### ユーザー向け手順（推奨）
1. `Publisher.cer` を開き、「ローカル コンピューター > 信頼された発行元」にインポート（管理者権限が必要）。
2. 企業/私設 CA を使用している場合は、その `CER` を「信頼されたルート証明機関」にもインポート。
3. バックエンドを HTTPS で利用する場合、自己署名を用いるなら `wrapper/scripts/make_dev_https_cert.ps1` で PFX/CER を作成し、GUI 側で cert/key を指定。クライアント端末には CER を信頼登録。

### 配布者向け: 同梱物・運用
- `Publisher.cer` を MSIX と同梱（ユーザーが最初に信頼登録）。
- `wrapper/scripts/install_publisher_cert.ps1` を同梱し、管理者 PowerShell で実行して証明書を自動インポート可能。
