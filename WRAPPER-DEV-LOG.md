...
## 2025-02-14 (ModelManagerDialogの翻訳関数追加と未使用コード削除)
- 背景／スコープ：ModelManagerDialogで翻訳メソッド `_t` が未定義のためエラーとなる可能性があった。また未使用のコードやAvalonia関連ディレクトリが残存していた。
- 決定事項：
  - WrapperGUIの翻訳関数をModelManagerDialogへ渡し、ダイアログ内でもローカライズを適用。
  - 未使用の`choose_vad_certfile`メソッドおよびAvalonia関連ディレクトリを削除。
- 根拠・検討メモ：不要コード削減と実行時エラー防止のため。
- 未解決事項：なし
- 次アクション：なし
- リスク／課題：なし
- 参照リンク：
