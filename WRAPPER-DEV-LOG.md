...
## 2025-09-26
- 背景／スコープ：Silero VAD や Hugging Face モデルを企業内プロキシ環境で取得する際、独自 CA 証明書の指定が必要になるケースがある。
- 決定事項：
  - GUI に `CA bundle for downloads` フィールドを追加し、証明書パスを設定可能とした。
  - 設定されたパスを `REQUESTS_CA_BUNDLE` と `SSL_CERT_FILE` に適用し、モデルダウンロードや VAD 取得時に利用する。
- 根拠・検討メモ：企業ネットワークでは GitHub などの証明書検証で失敗することがあるため、ユーザーが任意 CA を指定できる必要がある。
- 未解決事項：指定証明書の妥当性チェックやプリフライトテストの自動化。
- 次アクション：証明書プリフライトの実装検討。
- リスク／課題：誤った証明書指定によるダウンロード失敗、設定漏れ。
- 参照リンク：
  - `README-FOR-WRAPPER.md` 設定節、`wrapper/app/gui.py`
...
## 2025-09-15
- 背景／スコープ：VAD モデルを既存のモデル管理に統合し、モデル一覧の用途を明確化したい。
- 決定事項：
  - `model_manager` に Silero VAD を torch.hub で扱う機能を追加し、キャッシュを統一。
  - モデル管理ダイアログに種別（Whisper/Segmentation/Embedding/VAD）を表示。
- 根拠・検討メモ：オフライン運用時に VAD を事前取得でき、各モデルの用途が一目で分かるようになるため。
- 未解決事項：torch.hub ダウンロード時の進捗表示精度。
- 次アクション：利用者からのフィードバックを踏まえた UI 微調整。
- リスク／課題：VAD リポジトリ構成変更時のダウンロード失敗。
- 参照リンク：`README-FOR-WRAPPER.md` モデル管理節、`wrapper/app/model_manager.py`、`wrapper/app/gui.py`。
...
## 2025-09-16
- 背景／スコープ：VAD モデル未取得時の誤操作防止。
- 決定事項：
  - Silero VAD 未ダウンロード時に GUI の VAD チェックボックスを無効化し、モデル取得後に自動で有効化。
- 根拠・検討メモ：モデルが存在しない状態で VAD を有効化するとダウンロード失敗や実行時エラーとなるため、UI で抑止する。
- 未解決事項：モデル取得状況の自動監視や通知の精度向上。
- 次アクション：利用者フィードバックを集め、必要に応じて UX を調整。
- リスク／課題：モデル管理ダイアログを閉じ忘れた場合の状態更新遅延。
