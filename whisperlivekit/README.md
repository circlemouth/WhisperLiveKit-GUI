...
A basic Dockerfile is provided which allows re-use of installation options. See below useage examples:

#### All defaults
- Create a reuseable image with only the basics and then run as a named container.
```bash
docker build -t whisperlivekit-defaults .
docker create --name whisperlivekit -p 8000:8000 whisperlivekit-defaults
